	...
	<div class="item <?php echo esc_attr( $column ); ?> <?php echo esc_attr( $class_columns ); ?>">
		<div class="avatar-item">
			<div class="avatar-instructors">
				<?php
				$image   = get_avatar( $instructor["user_id"], 500 );
				$pattern = '/src="([^"]*)"/';
				...
				?>
				<?php if ( $image_crop ) { ?>
					<a class="avatar" href="<?php echo learn_press_user_profile_link( $instructor["user_id"] ); ?>">
						<img src="<?php echo esc_url( $image_crop ); ?>"
							 alt="<?php echo get_the_author_meta( 'display_name', $instructor["user_id"] ); ?>"/>
					</a>
				<?php } else { ?>
