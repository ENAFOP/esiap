...
<?php if ( $course ) { ?>
    ...
    <div class="course-students-list">
        ...
        <?php
        $students_list_heading     = apply_filters( 'learn_press_students_list_heading', __( 'Students List', 'course-builder' ) );
        ...
        $filters                   = apply_filters( 'learn_press_get_students_list_filter',
            array(
                ...
                'in-progress' => esc_html__( 'In Progress', 'course-builder' ),
                'finished'    => esc_html__( 'Finished', 'course-builder' )
            ) );
        ...
        <?php if ( $students = $curd->get_user_enrolled( $course->get_ID(), $limit ) ) { ?>
            <div class="filter-students">
                <label for="students-list-filter"><?php esc_html_e( 'Student filter', 'course-builder' ); ?></label>
        ...
        <?php if ( $students = $curd->get_user_enrolled( $course->get_ID(), $limit ) ) { ?>
                        ...
                        <div class="user-info">
                            ...
                            <div class="right-info">
                                ...
                                <div class="course-progress">
                                    <span class="course-result"><?php echo esc_html__( 'Learning', 'course-builder' );?></span><?php echo $result; ?>%
                                </div>
                            </div>
                        </div>
                    </li>
            ...
            if ( $other_student && $limit == - 1 ) {
                echo '<p class="additional-students">and ' . sprintf( _n( 'one student enrolled.', '%s students enrolled.', $other_student, 'course-builder' ), $other_student ) . '</p>';
            }
            ?>
        <?php } else { ?>
            <div class="students empty">
                <?php if ( $course->get_users_enrolled() ) {
                    echo apply_filters( 'learn_press_course_count_student', sprintf( _n( 'One student enrolled.', '%s students enrolled.', $course->get_users_enrolled(), 'course-builder' ), $course->get_users_enrolled() ) );
                } else {
                    echo apply_filters( 'learn_press_course_no_student', __( 'No student enrolled.', 'course-builder' ) );
                } ?>
            </div>
        <?php } ?>
    </div>
    ...
    echo __( 'Course ID invalid, please check it again.', 'course-builder' );
}